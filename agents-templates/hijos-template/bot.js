...
// Identify who is sending this message (Jose, Valentina/hijo, otros)
...
function formatUserPrefix(msg) {
  ...
  if (id === JOSE_USER_ID) return `[De Jose Navarro (papá, creador, user ID ${id})]`;
  ...
  if (msg.voice || msg.audio) {
    ...
    enqueueMessage(async () => {
      ...
      try {
        ...
        const senderPrefix = formatUserPrefix(msg);
        let rawResponse = await callAgent(`${senderPrefix} [Este mensaje viene de un audio] ${transcription}`);
        rawResponse = await handleDelegation(rawResponse);
        const responseText = rawResponse.replace(/^\[(AUDIO|TEXTO)\]\s*/i, '').trim();
...
async function handleTextMessage(chatId, text, senderMsg) {
  ...
  try {
    ...
    const payload = senderPrefix ? `${senderPrefix} ${text}` : text;
    let rawResponse = await callAgent(payload);
    rawResponse = await handleDelegation(rawResponse);
    const responseText = rawResponse.replace(/^\[(AUDIO|TEXTO)\]\s*/i, '').trim();
    await sendTextResponse(chatId, responseText);
